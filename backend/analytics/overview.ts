  ...
  async (req) => {
    ...
    const revenueQuery = `
      SELECT 
        ...
        COALESCE(AVG(b.checkout_date - b.checkin_date), 0) as avg_stay_length
      FROM properties p
      LEFT JOIN revenues r ON p.id = r.property_id AND r.occurred_at BETWEEN $2 AND $3
      LEFT JOIN bookings b ON p.id = b.property_id AND b.checkin_date BETWEEN $2 AND $3
    ...
    const expenseQuery = `
      ...
      FROM properties p
      LEFT JOIN expenses e ON p.id = e.property_id AND e.created_at BETWEEN $2 AND $3
      WHERE p.org_id = $1 ${propertyFilter}
    `;
    ...
    const occupancyQuery = `
      ...
      FROM properties p
      LEFT JOIN beds_or_units bu ON p.id = bu.property_id
      WHERE p.org_id = $1 ${propertyFilter}
    `;
    ...
    const taskQuery = `
      ...
      FROM properties p
      LEFT JOIN tasks t ON p.id = t.property_id AND t.created_at BETWEEN $2 AND $3
      WHERE p.org_id = $1 ${propertyFilter}
    `;
