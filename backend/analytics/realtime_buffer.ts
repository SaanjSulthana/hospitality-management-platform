...
type OrgBuffer = {
  ...
  waiters: Array<{
    sinceTime?: Date;
    propertyId?: number;
    resolve: (events: AnalyticsEventPayload[]) => void;
...
export function bufferAnalyticsEvent(event: AnalyticsEventPayload): void {
...
function getEventsSinceInternal(orgId: number, sinceTime?: Date, propertyId?: number): AnalyticsEventPayload[] {
  const buf = buffers.get(orgId);
  if (!buf) return [];
  cleanupExpired(buf);
...
export function getEventsSince(orgId: number, lastEventIdISO?: string, propertyId?: number): AnalyticsEventPayload[] {
  const sinceTime = lastEventIdISO ? new Date(lastEventIdISO) : undefined;
  return getEventsSinceInternal(orgId, sinceTime, propertyId);
}

export function waitForEvents(orgId: number, lastEventIdISO?: string, propertyId?: number): Promise<AnalyticsEventPayload[]> {
  const sinceTime = lastEventIdISO ? new Date(lastEventIdISO) : undefined;
  ...
  return new Promise<AnalyticsEventPayload[]>((resolve, reject) => {
