...
type OrgBuffer = {
  ...
  waiters: Array<{
    sinceTime?: Date;
    resolve: (events: BrandingEventPayload[]) => void;
...
export function bufferBrandingEvent(event: BrandingEventPayload): void {
...
function notifyWaiters(orgId: number): void {
  ...
  for (const w of waiters) {
    ...
    try {
      const events = getEventsSinceInternal(orgId, w.sinceTime);
      w.resolve(events);
    } catch (e: any) {
      w.reject(e);
    }
...
function getEventsSinceInternal(orgId: number, sinceTime?: Date): BrandingEventPayload[] {
  const buf = buffers.get(orgId);
  if (!buf) return [];
  cleanupExpired(buf);
...
export function getEventsSince(orgId: number, lastEventIdISO?: string): BrandingEventPayload[] {
  const sinceTime = lastEventIdISO ? new Date(lastEventIdISO) : undefined;
  return getEventsSinceInternal(orgId, sinceTime);
}

export function waitForEvents(orgId: number, lastEventIdISO?: string): Promise<BrandingEventPayload[]> {
  const sinceTime = lastEventIdISO ? new Date(lastEventIdISO) : undefined;
  const buf = getOrCreateBuffer(orgId);
  const available = getEventsSinceInternal(orgId, sinceTime);
  if (available.length > 0) {
    return Promise.resolve(available);
  }
  return new Promise<BrandingEventPayload[]>((resolve, reject) => {
    ...
    buf.waiters.push({ sinceTime, resolve, reject, timeout });
    buf.lastActive = Date.now();
  });
}
