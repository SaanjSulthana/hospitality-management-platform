...
type OrgBuffer = {
  ...
  waiters: Array<{
    sinceTime?: Date;
    propertyId?: number;
    resolve: (events: DashboardEventPayload[]) => void;
...
export function bufferDashboardEvent(event: DashboardEventPayload): void {
...
function getEventsSinceInternal(orgId: number, sinceTime?: Date, propertyId?: number): DashboardEventPayload[] {
  const buf = buffers.get(orgId);
  if (!buf) return [];
  cleanupExpired(buf);
  const events = buf.queue
    ...
    .map(item => item.event)
    .filter(e => (propertyId ? (e.propertyId ?? null) === propertyId : true));
...
export function getEventsSince(orgId: number, lastEventIdISO?: string, propertyId?: number): DashboardEventPayload[] {
  const sinceTime = lastEventIdISO ? new Date(lastEventIdISO) : undefined;
  return getEventsSinceInternal(orgId, sinceTime, propertyId);
}

export function waitForEvents(orgId: number, lastEventIdISO?: string, propertyId?: number): Promise<DashboardEventPayload[]> {
  const sinceTime = lastEventIdISO ? new Date(lastEventIdISO) : undefined;
  ...
  return new Promise<DashboardEventPayload[]>((resolve, reject) => {
