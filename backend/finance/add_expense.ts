...
} from "../middleware/idempotency_redis";
...
async function addExpenseHandler(req: AddExpenseRequest): Promise<AddExpenseResponse> {
  ...
  if (!rateLimitResult.allowed) {
    console.log('[Finance] Rate limit exceeded for expense creation:', { userId, orgId });
  ...
  if (idempotency.shouldReturn && idempotency.response) {
    console.log('[Finance] Idempotency replay for expense:', idempotencyKey);
    return idempotency.response.body as AddExpenseResponse;
  }
  ...
  try {
    ...
    try {
      await financeEvents.publish({
        ...
        metadata: {
          ...
          description: expenseRow.description || undefined
        }
      });
    ...
    try {
      triggerPurgeOnMutation('finance', authData.orgId, expenseRow.property_id, expenseRow.id);
      triggerPurgeOnMutation('reports', authData.orgId, expenseRow.property_id);
      console.log(`[Finance] Triggered CDN purge for expense ID: ${expenseRow.id}`);
    } catch (purgeError) {
