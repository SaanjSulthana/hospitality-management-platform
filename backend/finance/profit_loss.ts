...
export const profitLoss = api<ProfitLossRequest, ProfitLossResponse>(
  ...
  async (req) => {
    ...
    const expenseQuery = `
      SELECT 
        COALESCE(SUM(e.amount_cents), 0) as total_expenses_cents,
        e.category,
        COALESCE(SUM(e.amount_cents), 0) as category_amount_cents
      FROM properties p
      LEFT JOIN expenses e ON p.id = e.property_id AND e.expense_date BETWEEN $2 AND $3 AND e.status = 'approved'
      WHERE p.org_id = $1 ${propertyFilter}
      GROUP BY e.category
    `;
