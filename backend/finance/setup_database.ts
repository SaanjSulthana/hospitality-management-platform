...
export const setupDatabase = api(
  { auth: true, expose: true, method: "POST", path: "/finance/setup-database" },
  async () => {
    ...
    try {
      console.log("Setting up finance database tables...");
      ...
      throw APIError.internal(`Database setup failed: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
);
