...
type OrgBuffer = {
  ...
  waiters: Array<{
    sinceTime?: Date;
    propertyId?: number;
    resolve: (events: PropertyEventPayload[]) => void;
...
export function bufferPropertyEvent(event: PropertyEventPayload): void {
...
function getEventsSinceInternal(orgId: number, sinceTime?: Date, propertyId?: number): PropertyEventPayload[] {
  const buf = buffers.get(orgId);
  if (!buf) return [];
  cleanupExpired(buf);
...
export function getEventsSince(orgId: number, lastEventIdISO?: string, propertyId?: number): PropertyEventPayload[] {
  const sinceTime = lastEventIdISO ? new Date(lastEventIdISO) : undefined;
  return getEventsSinceInternal(orgId, sinceTime, propertyId);
}

export function waitForEvents(orgId: number, lastEventIdISO?: string, propertyId?: number): Promise<PropertyEventPayload[]> {
  const sinceTime = lastEventIdISO ? new Date(lastEventIdISO) : undefined;
  ...
  return new Promise<PropertyEventPayload[]>((resolve, reject) => {
