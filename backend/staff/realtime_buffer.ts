...
type OrgBuffer = {
  ...
  waiters: Array<{
    sinceTime?: Date;
    propertyId?: number;
    resolve: (events: StaffEventPayload[]) => void;
...
export function bufferStaffEvent(event: StaffEventPayload): void {
...
function getEventsSinceInternal(orgId: number, sinceTime?: Date, propertyId?: number): StaffEventPayload[] {
  const buf = buffers.get(orgId);
  if (!buf) return [];
...
export function getEventsSince(orgId: number, lastEventIdISO?: string, propertyId?: number): StaffEventPayload[] {
  const sinceTime = lastEventIdISO ? new Date(lastEventIdISO) : undefined;
  return getEventsSinceInternal(orgId, sinceTime, propertyId);
}

export function waitForEvents(orgId: number, lastEventIdISO?: string, propertyId?: number): Promise<StaffEventPayload[]> {
  const sinceTime = lastEventIdISO ? new Date(lastEventIdISO) : undefined;
  const buf = getOrCreateBuffer(orgId);

  const available = getEventsSinceInternal(orgId, sinceTime, propertyId);
  if (available.length > 0) {
    console.log(`[StaffRealtimeBuffer] have_data org=${orgId} count=${available.length}`);
    return Promise.resolve(available);
  }

  return new Promise<StaffEventPayload[]>((resolve, reject) => {
    if (buf.waiters.length >= MAX_WAITERS_PER_ORG) {
      console.warn(`[StaffRealtimeBuffer] waiter_cap org=${orgId} waiters=${buf.waiters.length} cap=${MAX_WAITERS_PER_ORG}`);
      try {
        resolve([]);
      } catch (e) {
        reject(e as Error);
    ...
    const timeout = setTimeout(() => {
      try {
        console.log(`[StaffRealtimeBuffer] timeout org=${orgId} ms=${LONG_POLL_TIMEOUT_MS}`);
        resolve([]);
      } catch (e) {
        reject(e as Error);
      }
