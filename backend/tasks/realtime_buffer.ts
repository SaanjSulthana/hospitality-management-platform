...
type OrgBuffer = {
  ...
  waiters: Array<{
    sinceTime?: Date;
    propertyId?: number;
    resolve: (events: TaskEventPayload[]) => void;
...
export function bufferTaskEvent(event: TaskEventPayload): void {
...
function getEventsSinceInternal(orgId: number, sinceTime?: Date, propertyId?: number): TaskEventPayload[] {
  const buf = buffers.get(orgId);
  if (!buf) return [];
  cleanupExpired(buf);
...
export function getEventsSince(orgId: number, lastEventIdISO?: string, propertyId?: number): TaskEventPayload[] {
  const sinceTime = lastEventIdISO ? new Date(lastEventIdISO) : undefined;
  return getEventsSinceInternal(orgId, sinceTime, propertyId);
}

export function waitForEvents(orgId: number, lastEventIdISO?: string, propertyId?: number): Promise<TaskEventPayload[]> {
  const sinceTime = lastEventIdISO ? new Date(lastEventIdISO) : undefined;
  ...
  return new Promise<TaskEventPayload[]>((resolve, reject) => {
