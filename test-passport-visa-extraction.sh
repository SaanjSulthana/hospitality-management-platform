...
# This script tests with the actual passport and visa images provided
...
test_real_document() {
  ...
  if [ $FILE_SIZE -gt $((500 * 1024 * 1024)) ]; then
    echo -e "${RED}✗ File too large: ${FILE_SIZE_MB}MB (max 500MB)${NC}"
    return 1
  fi
...
main() {
  ...
  # Test with the provided passport and visa images
  echo ""
  echo "Testing with provided document images..."
  
  # Test Estonian passport
  if [ -f "passport.jpg" ]; then
    test_real_document "passport.jpg" "passport" "Estonian Passport Extraction"
  else
    echo -e "${YELLOW}⚠ passport.jpg not found in current directory${NC}"
    echo "Please ensure the Estonian passport image is named 'passport.jpg' in the project root"
  fi
  
  # Test Indian visa
  if [ -f "visa.jpg" ]; then
    test_real_document "visa.jpg" "visa_front" "Indian Visa Extraction"
  else
    echo -e "${YELLOW}⚠ visa.jpg not found in current directory${NC}"
    echo "Please ensure the Indian visa image is named 'visa.jpg' in the project root"
  fi
  ...
  echo "• ✅ File size limits configured (500MB)"
  echo "• ✅ LLM extraction system ready"
  echo ""
  echo "Next Steps:"
  echo "1. If extraction failed, check OpenAI API key configuration"
  echo "2. Upload documents through the frontend UI"
  echo "3. The system will automatically extract and fill data"
  echo "4. Review extracted fields and confidence scores"
  echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
}
