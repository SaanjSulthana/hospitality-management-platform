...
# Tests the complete upload pipeline without requiring actual image files
...
create_test_image() {
    ...
    # This is a valid JPEG header + minimal data
...
test_file_size_limits() {
    ...
    echo -e "${YELLOW}📏 Testing file size limits...${NC}"
    
    # Create a test file that's too large (simulate 9MB)
    echo -e "${CYAN}Testing with oversized file...${NC}"
    
    # Create a large base64 string (simulating 9MB file)
    local large_base64=$(printf 'A%.0s' {1..12000000})
...
EOF
    ...
    echo -e "${YELLOW}📤 Attempting upload of large file...${NC}"
    local upload_response=$(make_request "POST" "/guest-checkin/documents/upload" "$upload_data" "$token")
    echo "Large file upload response: $upload_response"
    
    if echo "$upload_response" | grep -q '"success":true'; then
        echo -e "${YELLOW}⚠️  Large file upload succeeded (may need to adjust limits)${NC}"
    else
        echo -e "${GREEN}✅ Large file upload properly rejected${NC}"
    fi
}
...
# Step 7: Test file size limits
echo -e "${BLUE}Step 7: Testing File Size Limits${NC}"
echo "--------------------------------"

test_file_size_limits "$TOKEN" "$ELECTION_GUEST"
echo ""
